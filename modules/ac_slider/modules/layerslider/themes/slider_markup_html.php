...
if(!empty($slider['slides']) && is_array($slider['slides'])) {
	foreach($slider['slides'] as $slidekey => $slide) {
		...
		if(!empty($slide['props']['background'])) {
			$src = !empty($slide['props']['backgroundId']) ? ls_get_image((int)$slide['props']['backgroundId']) : $slide['props']['background'];
			$data[] = '<img src="'.file_create_url(acquia_variable_get('layerslider_root').'/static/img/blank.gif') .'" data-src="'.$src.'" class="ls-bg" alt="Slide background" />';
		}
		...
		if(isset($slides['properties']['attrs']['thumbnailNavigation'])) {
			if($slides['properties']['attrs']['thumbnailNavigation'] != 'disabled') {
				if(!empty($slide['props']['thumbnail'])) {
					...
					$data[] = '<img src="'.file_create_url(acquia_variable_get('layerslider_root').'/static/img/blank.gif').'" data-src="'.$src.'" class="ls-tn" alt="Slide thumbnail" />';
				}
			}
		}
		...
		if(!empty($slide['layers']) && is_array($slide['layers'])) {
			foreach($slide['layers'] as $layerkey => $layer) {
				...
				if($layer['props']['type'] == 'img') {
					$inner->attr('src', file_create_url(acquia_variable_get('layerslider_root').'/static/img/blank.gif'));
