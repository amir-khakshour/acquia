 ...
 * =========================================================
 * Copyright 2012 dbakery
 *
 * Visual composer javascript functions to enable fields.
 * This script loads with settings form.
 * ========================================================= */

var db_change_tab_title, db_change_accordion_tab_title;

!function($) {
    db_change_tab_title = function($element, field) {
        $('.tabs_controls a[href=#tab-' + $(field).val() +']').text($('.db-edit-form [name=title].db-param-val').val());
    };
    db_change_accordion_tab_title = function($element, field) {
    ...
    function init_textarea_html_old($element) {
        ...
        $form_line.find('.wp-switch-editor').removeAttr("onclick");
         ...
         $('#db_tinymce_content-html').trigger('click');
         $('#db_tinymce_content-tmce').trigger('click'); // Fix hidden toolbar
    }
    $('.db-edit-form .textarea_html').each(function(){
    ...
    var VcLoop = Backbone.View.extend({
        ...
        initialize: function() {
            ...
            this.$input = $('.db-param-val', this.$el);
    ...
    var VcOptionsField = Backbone.View.extend({
        ...
        initialize: function() {
            ...
            this.$input = this.$el.find('.db-param-val');
    ...
    $(function(){
        $('.db_el_type_loop').each(function(){
            new VcLoop({el: $(this).get(0)});
        });
        $('.db_el_type_options').each(function(){
