...
jQuery(document).ready(function ($) {
    $('.db_settings_accordion').accordion({
        active:(getCookie('db_js_composer_settings_group_tab') ? getCookie('db_js_composer_settings_group_tab') : false),
        collapsible:true,
        change:function (event, ui) {
            if (ui.newHeader.attr('id') !== undefined)
                setCookie('db_js_composer_settings_group_tab', '#' + ui.newHeader.attr('id'), 365 * 24 * 60 * 60);
            else
                setCookie('db_js_composer_settings_group_tab', '', 365 * 24 * 60 * 60);
        }
    });
    $('.db-settings-select-all-shortcodes').click(function (e) {
        e.preventDefault();
        $(this).parent().parent().find('[type=checkbox]').attr('checked', true);
    });
    $('.db-settings-select-none-shortcodes').click(function (e) {
        e.preventDefault();
    ...
    $('.vc-settings-tab-content').submit(function () {
        setCookie('db_js_composer_settings_active_tab', $('.vc-settings-tab-control.nav-tab-active').attr('href'), 365 * 24 * 60 * 60);
        return true;
    });
    ...
    $('#vc-settings-disable-notification-button').click(function (e) {
        ...
        $.ajax({
            type:'POST',
            url:window.ajaxurl,
            data:{action:'db_remove_settings_notification_element_css_class'}
        });
        $(this).remove();
    });
    ...
    $('#db_js_use_custom').change(function () {
        if ($(this).is(':checked')) {
